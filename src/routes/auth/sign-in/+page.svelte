...
<AuthLayout title="Sign in" subtitle="Welcome back to AsyncHub">
  <form onsubmit={handleSubmit} class="space-y-1">
    ...
    <Button type="submit" loading={loading} disabled={!isFormValid} class="mt-4">Sign in</Button>
  </form>
  ...
  <div class="grid grid-cols-2 gap-3">
    <button onclick={() => signIn.social({ provider: 'github', callbackURL: '/dashboard' })} 
            class="flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-300 rounded-xl hover:bg-gray-50 transition">
  ...
  <p class="mt-6 text-center text-sm text-gray-600">
    ...
    <a href="/auth/sign-up" class="font-medium text-gray-900 hover:underline">Create one</a>
  </p>
</AuthLayout>
